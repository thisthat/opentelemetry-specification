...
Deployments of cloud-native application is automated and done at fast phase.
...
This document defines semantic conventions to collect telemetry signals about events that occur inside the
...
Each stage of the lifecycle of a cloud-native application deployment SHOULD be recorded as Event on the span during 
which it occurred.
...
Cloud-native applications are composed by one or several components that are usually called [workloads](https://kubernetes.io/docs/concepts/workloads/).
Similarly to applications, each stage of the lifecycle of a workload deployment SHOULD be recorded as Event on the span during
which it occurred.
